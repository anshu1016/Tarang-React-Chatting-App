        ...
        <div className="grid grid-cols-2 ">
          <div className="h-full w-32 md:w-48 md:h-48 relative flex items-center justify-center " onMouseEnter={()=>setHovered(true)} onMouseLeave={()=>setHovered(false)} >
            <Avatar className='h-32 w-32 md:w-48 md:h-48 rounded-full overflow-hidden '>
              {image ? <AvatarImage src={image} alt="profile pic" className='object-cover w-full h-full bg-black'/> : <div className={`uppercase h-32 w-32 md:h-48 md:w-48 text-5xl border-[1px] flex items-center justify-center rounded-full ${getColor(selectedColor)}`}>{firstName ? firstName.split("").shift() : userInfo.email.split("").shift()}</div>}
            </Avatar>
            {
            ...
            <div className="w-full flex gap-5 ">
              {
                ...
                onChange={()=>setSelectedColor(index)}></div>)
              }
            </div>
          </div>
        </div>
              <div className="w-full">
                ...
                onChange={saveChanges}>Save Changes</Button>
              </div>
      </div>
    </div>
  )
